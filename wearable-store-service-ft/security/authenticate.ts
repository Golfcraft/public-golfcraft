...
const catalustUrlTest = "https://peer.decentraland.zone/lambdas/crypto/validate-signature";
...
function validSignatureInterval(timestamp: number) {
  console.log("validSignatureInterval", typeof timestamp, timestamp)
...
export async function authenticateUsingAuthChain(
  ...
  if (chain.length === 0) {
    ...
    const resp = await fetch(
        ~(metadataJSON.realm.domain || metadataJSON.realm.hostname).indexOf("testing")
            ? catalustUrlTest
            : catalystUrl,
        {
          method: 'post',
