...
export class NFTdata{
  ...
  hoverText: string
  bDebug: boolean
  constructor(entity: IEntity, smartContract, tokenId, title?: string, autor?: string, description?: string, owner?: string, hoverText?: string, frame?: number){
    this.entity = entity
    ...
    if (smartContract!="") {
      ...
      if (entity.hasComponent(GLTFShape) || entity.hasComponent(BoxShape) || entity.hasComponent(SphereShape)) {
        ...
        entityNFT.addComponent(new Transform({
          ...
          scale: new Vector3(entity.getParent().getComponent(Transform).scale.y/entity.getParent().getComponent(Transform).scale.x,1,1),
        }))
        entityNFT.addComponent(shapeComponent)
        entityNFT.setParent(entity.getParent())
...
export class WidgetNFT extends Widget{
  ...
  constructor(parentUI: Widget | UIShape){
    ...
    var image = new UIImage(parent, new Texture("assets/UI_ArtDescription_02.png"))
    image.visible = false
