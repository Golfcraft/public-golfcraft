...
export class Robot{
  ...
  constructor(entity: IEntity, dialogIndex: number){
    ...
    this.entityWalking.addComponent(new GLTFShape("assets/archive/robot/robot_walking.gltf"))
    ...
    this.entityTalking.addComponent(new GLTFShape("assets/archive/robot/robot_talking.gltf"))
  ...
  setTalkMode(){
    ...
    var self = this
    delay(() => {
      getHUD().setWidgetDialogIndex(getHUD().wgTalk, this.dialogIndex)
      getHUD().wgTalk.callback = function(){
        getHUD().hideAll()
        delay(() => {
          self.stopLookAt()
          self.setMoveMode()
