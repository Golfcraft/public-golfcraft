...
describe("GolfcraftMaterials MetaTransactions", function () {
  let golfcraftmaterials;

    before(async () => {
        const GolfcraftMaterials = await ethers.getContractFactory("GolfcraftMaterials");
        golfcraftmaterials = await upgrades.deployProxy(GolfcraftMaterials, ["https://tokenuris/", "GolfcraftMaterials", "GCFTMTRLS", "https://contracturi/"]);
  ...
  it("mint MetaTransaction Test", async function () {
    ...
    let name = await golfcraftmaterials.name();
    let nonce = await golfcraftmaterials.getNonce(wallet.getAddress());
    let version = "1";
    let chainId = await golfcraftmaterials.getChainId();
    let domainData = {
      name: name,
      version: version,
      verifyingContract: golfcraftmaterials.address,
    ...
    const mintTx = await golfcraftmaterials.mint(user, 0, 10, []);
    await mintTx.wait();

    expect((await golfcraftmaterials.balanceOf(account1.address, 0)).toString()).to.be.equal("0");

    const metaTransaction = await golfcraftmaterials.executeMetaTransaction(
      user,
      functionSignature,
      r,
      s,
    ...
    expect((await golfcraftmaterials.balanceOf(account1.address, 0)).toString()).to.be.equal("5");
  });
});
