...
describe("WSDeployer (Storage)", function () {
    let wsdeployer;
    let wearable;
    let erc721;
    ...
    it("Should return the address of the new contract", async function () {
        ...
        await expect(deployWearablesStorageTX).to.emit(wsdeployer, 'ContractCreated')
        ...
        expect(collectionName).to.equal(await storage.collectionName());
    ...
    it("Should be able to send NFTs", async function () {
        ...
        const sendNFTTX = await storage.sendNFT(erc721.address, accounts[1].address);
        await sendNFTTX.wait();

        const balance2 = await erc721.balanceOf(accounts[1].address);
        expect(balance2).to.equal(1);

        const balance3 = await erc721.balanceOf(storage.address);
    ...
    it("Should be able to send NFTs from", async function () {
        ...
        const sendNFTTX = await storage.sendNFTFrom(erc721.address, accounts[5].address, accounts[1].address);
        await sendNFTTX.wait();
    ...
    it("Should be able to relay issueTokens", async function () {
        ...
        const relayIssueTokensTXr = storage.connect(accounts[10]).relayIssueTokens(wearable.address, [accounts[0].address], [0]);
        await expect(relayIssueTokensTXr).to.be.revertedWith('WearablesStorage: Sender should be admin');

        const relayIssueTokensTX = await storage.relayIssueTokens(wearable.address, [accounts[0].address], [0]);
    ...
    it("Should be able to handle metatransactions", async function () {
        ...
        //const WearablesStorage = await ethers.getContractFactory("WearablesStorage");
        const ABI = WearablesStorage.interface.format(ethers.utils.FormatTypes.full);
        ...
        let domainData = {
          ...
          version: "3",
          salt: '0x' + (setApprovalForAllTX.chainId).toString(16).padStart(64, '0'),
        ...
        const executeMetaTransactionTX = await storage.executeMetaTransaction(accounts[0].address, functionSignature, r, s, v);
    ...
    it("Should be able to setAprovalForAll", async function () {
        ...
        const setAprovalForAllTX = await storage.setApprovalForAll(erc721.address, accounts[0].address, true);
        await setAprovalForAllTX.wait();

        const safeTransferFromTX = await erc721['safeTransferFrom(address,address,uint256)'](storage.address, accounts[1].address, tokenId);
