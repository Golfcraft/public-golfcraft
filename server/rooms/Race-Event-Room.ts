...
export class RaceEventRoom extends Room<PlayState> {
    ...
    async onCreate({realm, user, userId, gameDefinition, courseDefinition, cannonParts, timeStep = 1/(60*4)}) {//TODO client shouldn't define gameDefinition, it should inherit from currentTrainingID from player
            console.log("created", {timeStep, user});
//TODO get race from
            if(gameDefinition){
        ...
        this.setSimulationInterval((dt) => {
            ...
            this.golfPlay.onEvent(async ({type, data}) => {
                if(type === EVENT.SLEEP){
                    ...
                    if( index === (checkpointsLength-1) ) {
                        ...
                        //TODO send
                        const time = Date.now()-this.state.ballStartTimes[id];
                        try{
                            const result = await promisify(PlayFabServer.UpdatePlayerStatistics)({
                                PlayFabId:this.state.ballPlayFabIdMap[id],
                                Statistics:[{ StatisticName:"Ninja", Value:999999999-time}
                                ]
                            });
                            console.log("result statistics Ninja", result);
                            await sleep(100);
                            const resultD = await promisify(PlayFabServer.AddUserVirtualCurrency)({
                                VirtualCurrency:"DM",
                                Amount:1,
                                PlayFabId:this.state.ballPlayFabIdMap[id],
                            });
                        ...
                        foundClient.send(MESSAGE.COMPLETED, {time});
                        //TODO dispose everything
                    } else {
                        foundClient.send(MESSAGE.CHECKPOINT, {checkpoint, index});
                    }
            ...
            this.onMessage(MESSAGE.SHOOT, async (client, {impulse, timeStep, delayMs})=>{
                ...
                if(this.state.ballStartTimes[id] >= Date.now()) return;
                await sleep(200);
    ...
    async onJoin(client, {realm, user, userId, PlayFabId, gameDefinition, courseDefinition, cannonParts, displayName}){
        //TODO add new ball
        console.log("joining1", Date.now());
        await waitFor(()=>!!this.golfPlay, 100);
        console.log("joining2", Date.now())
        this.ballIds++;
        const ballId = this.ballIds.toString();
        this.state.ballStartTimes[ballId] = Date.now() + 5000;
        ...
        client.send(MESSAGE.START, {startTime:this.state.ballStartTimes[ballId], serverTime:Date.now()});
