...
exports.withIdsAsString = exports.withAnimationFramesCourse = exports.withFramesAsMsCourse = exports.migrateCourseDefinitionAnimations = void 0;
const utils_1 = require("./utils");
function migrateCourseDefinitionAnimations(courseDefinition) {
    return utils_1.pipe(withFramesAsMsCourse, withAnimationFramesCourse, withCompactCurvesCourse, withCourseAnimationKinematics, withIdsAsString, utils_1.deepFreeze)(courseDefinition);
}
exports.migrateCourseDefinitionAnimations = migrateCourseDefinitionAnimations;
function withFramesAsMsCourse(_courseDefinition, frameSeconds = 1 / 24) {
...
exports.withFramesAsMsCourse = withFramesAsMsCourse;
function withCompactCurves(transform) {
...
function withAnimationFramesCourse(_courseDefinition) {
    const courseDefinition = JSON.parse(JSON.stringify(_courseDefinition));
    courseDefinition.parts = courseDefinition.parts.map((part) => {
        if (part.animation) {
            part.animationFrameTimes = Object.values(part.animation)[0].x.map(i => i[0]);
        }
        return part;
    });
    return courseDefinition;
}
exports.withAnimationFramesCourse = withAnimationFramesCourse;
    ...
    return frames.map((_, index) => {
        return {
            ...
            angularVelocity: compactedAnimationPart.animation.rotation_quaternion
                ? getAngularVelocity(compactedAnimationPart.animation.rotation_quaternion[index], (index === (frames.length - 1))
                    ? getAllPropertiesWithZero(compactedAnimationPart.animation.rotation_quaternion[index])
                    : compactedAnimationPart.animation.rotation_quaternion[getNextIndex(index, compactedAnimationPart.animation.rotation_quaternion)], frames[getNextIndex(index, frames)] - frames[index])
                : { x: 0, y: 0, z: 0 }
        };
    });
}
...
function withIdsAsString(_courseDefinition) {
    const courseDefinition = JSON.parse(JSON.stringify(_courseDefinition));
    courseDefinition.parts = courseDefinition.parts.map(part => (Object.assign(Object.assign({}, part), { id: part.id.toString() })));
    return courseDefinition;
}
exports.withIdsAsString = withIdsAsString;
