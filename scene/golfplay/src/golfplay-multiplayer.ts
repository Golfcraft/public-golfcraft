...
export async function createMultiplayerGameplay(remote, remoteGameDefinition, parent, PlayFabId, realm = "unknown", roomInstanceId){
    const callbacks = {
        onFinish:null
    }
    try{
    ...
    console.log("loading courseDefinition ", JSON.stringify(gameDefinition))
    const courseDefinition = migrateCourseDefinitionAnimations(await getCourseDefinition(gameDefinition, fetch, true));
    console.log("courseDefinition displayNAme", courseDefinition.displayName, courseDefinition.authorName);
    ...
    console.log("joining roomInstanceId",roomInstanceId);
    const room = await colyseus.join(`demo-room`, {
        roomInstanceId,
        user:{
            ...user,
            avatar:undefined},
        realm, userId, PlayFabId,
        displayName,
...
console.log("room.state.balls[_ball.id].position.x",room.state.balls[_ball.id].position.x)
    ...
    room.onMessage(MESSAGE.HOLE, ({id})=>{
        console.log("HOLE", id)
        gamePlayState.finished = true;
    });
    ...
    room.onMessage(MESSAGE.OUT, ()=>{
        console.log("OUT");
        setTimeout(100, ()=>{
            console.log("handling OUT", JSON.stringify(gamePlayState))
    ...
    room.onMessage(MESSAGE.HOLE, ()=>{
        //TODO now we should wait for room to dispose (others to finish or 20s timeout)
    ...
    return {
        ...
        getCourseDefinition:()=>courseDefinition
    };
