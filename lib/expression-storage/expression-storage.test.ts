...
const spy = chai["spy"];
//TODO ADD RANDOMIZATION -> initialState:{ seed:tournamentCode } , then provide getRandomInt fn to jsep context
//TODO ADD CLOCK BY SECONDS: elapsedSeconds -> can be done out of the service with setInterval( () => rt.setState({elapsedSecods:Math.floor((Date.now() - startTime) / 1000)})
//TODO think how to sync state on room -> EVENT_VARIABLE_CHANGE > visualCourse.applyReactiveValue
//TODO ADD TEXT PART, where value can be any text, but expressions are moustache wrapped

import { EVENT } from './expression-storage';
import {createExpressionStorage} from "./expression-storage";

describe("ExpressionStorage", () => {
    let eStorage;

    beforeEach(()=>{
        eStorage = createExpressionStorage({a:1});
    });
    ...
    describe("addRuntimeAssignment", () => {
        beforeEach(()=>{
            eStorage = createExpressionStorage({});
        });
    ...
    describe("default context functions", ()=>{
        describe("getRandomInt(min,max)", ()=>{
            it("should evaluate a random seed and give different result on sequential calls", ()=>{
                const rt = createExpressionStorage({seed:0.1});
        ...
        describe("now()", ()=>{
           it("should return current datetime", ()=>{
               const rt = createExpressionStorage({});
               const control = rt.addControl({
                   id:1,
                   runtime:{
                       read:"now()"
